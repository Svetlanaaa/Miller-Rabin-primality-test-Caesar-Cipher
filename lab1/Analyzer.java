...
class Analyzer {
    ...
    private List<Character> sortValues(Map<Character, Integer> map) {
    ...
    Map<Character, Character> performAnalysis(Map<Character, Integer> table1, Map<Character, Integer> table2) {
        List<Character> list1 = sortValues(table1);
        List<Character> list2 = sortValues(table2);
        ...
        for (int i = 0; i < TOP_BIGRAMS_AMOUNT; i++) {
            for (int j = 0; j < 2; j++) {
                if (table.get(bigrams1Top.get(i).charAt(j)) != table.get(bigrams2Top.get(i).charAt(j))) {
                    table.replace(
                            bigrams1Top.get(i).charAt(j),
                            table.get(bigrams1Top.get(i).charAt(j)),
